...
OUTPUT_DIR   = os.getenv("OUTPUT_DIR", "./banking-model-gemma4-12b_full_lora")
...
# 2. Context & Precision (memory-safe defaults for this 80GB A100)
MAX_SEQ_LEN   = int(os.getenv("MAX_SEQ_LEN", "1024"))
LOAD_IN_4BIT  = True        # QLoRA keeps the base model in memory while training only the adapters
...
# Enable gradient checkpointing on HF models to reduce memory usage if supported
try:
...
class TrainingLogAccuracyCallback(TrainerCallback):
    """Print a sampled QA accuracy/F1 proxy while training."""

    def __init__(self, sample_size: int = 8, log_every: int = 10, max_new_tokens: int = 64):
        self.sample_size = sample_size
        self.log_every = log_every
        self.max_new_tokens = max_new_tokens

    def _sampled_accuracy(self):
        pairs = []
        for record in dataset.select(range(min(self.sample_size, len(dataset)))):
            text = record.get("text", "")
            if not isinstance(text, str):
                continue
            try:
                question = text.split("<start_of_turn>user\n", 1)[1].split("<end_of_turn>", 1)[0].strip()
        ...
        for question, answer in pairs:
            ...
            pred_words = {w for w in prediction.lower().split() if w}
            target_words = {w for w in answer.lower().split() if w}
            overlap = len(pred_words & target_words)
            precision = overlap / len(pred_words) if pred_words else 0.0
            recall = overlap / len(target_words) if target_words else 0.0
            f1 = 2 * precision * recall / (precision + recall) if precision + recall else 0.0
            scores.append(int(prediction.lower() == answer.lower()))
            f1_scores.append(f1)
        return sum(scores) / len(scores), sum(f1_scores) / len(f1_scores)

    def on_log(self, args, state, control, logs=None, **kwargs):
        if state.global_step == 0 or state.global_step % self.log_every != 0:
            return
        result = self._sampled_accuracy()
        if result is None:
            return
        accuracy, f1 = result
        print(
...
trainer.add_callback(TrainingLogAccuracyCallback(sample_size=8, log_every=10, max_new_tokens=64))
